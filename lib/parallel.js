(function () {
            ...
            if (typeof this.options.onMessage === 'function') {
                return [
                    ...
                    'self.onmessage = function(e) {',
                        'var global = {};',
                        'global.', ns, ' = ', env, ';',
                        'global.send = function(name, data) {',
	...
	Parallel.prototype.spawn = function (cb, env) {
		...
		this.operation.then(function () {
			...
			if (wrk !== undefined) {
                if (typeof that.options.onMessage === 'function') {
                    wrk.onmessage = function (msg) {
                        if (msg.data.name === 'complete') {
                            onComplete(msg.data.data);
                        } else {
                            that.options.onMessage(msg.data);
                        }
                    };
                } else {
                    wrk.onmessage = onComplete;
                }
